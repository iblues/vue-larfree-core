<template>
  <div class="remote">
    <component :is="currentView" v-bind="$props" />
      <lar-chart-line :model="test.test.line"></lar-chart-line>
  </div>
</template>
<script>
...
export default {
  ...
  computed: {
    currentView() {
      ...
      const ponentObj = new Function(`return ${tplData.sctipts.slice(tplData.sctipts.indexOf('{'), tplData.sctipts.lastIndexOf('}') + 1)}`)()
  ...
  methods: {
    ...
    resolveCss(lessInput) {
      less.render(lessInput).then(function(output) {
        ...
        { style.styleSheet.cssText = output.css } else { // w3c
    ...
    async getData() {
      const remoteData = this.$store.getters.getRemoteByUrl(this.url)
      if (remoteData) {
